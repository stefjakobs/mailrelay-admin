...
if (sizeof ($domain_properties) > 0)
{
   ...
   print "   <tr>\n";
   print "      <td colspan=\"8\"><h3>".$PALANG['pAdminList_domain_title']."</h3></td>";
   print "   </tr>";
   print "   <tr class=\"header\">\n";
   print "      <td>" . $PALANG['pAdminList_domain_domain'] . "</td>\n";
   print "      <td>" . $PALANG['pAdminList_domain_description'] . "</td>\n";
   if ($CONF['virtual'] == 'YES') print "      <td>" . $PALANG['pAdminList_domain_aliases'] . "</td>\n";
   if ($CONF['virtual'] == 'YES') {
      print "      <td>" . $PALANG['pAdminList_domain_mailboxes'] . "</td>\n";
      print "      <td>" . $PALANG['pAdminList_domain_maxquota'] . "</td>\n";
   }
   if ($CONF['transport'] == 'YES') print "      <td>" . $PALANG['pAdminList_domain_transport'] . "</td>\n";
   if ($CONF['relay'] == 'YES') print "      <td>" . $PALANG['pAdminList_domain_relay'] . "</td>\n";
...
#   for ($i = 0; $i < sizeof ($domain_properties); $i++)
   foreach(array_keys($domain_properties) as $i)
   {
      if ((is_array ($domain_properties) and sizeof ($domain_properties) > 0))
      {
         ...
         if ($CONF['relay'] == 'YES') print "<td align=right><a href=\"edit-transport.php?id=" . $domain_properties[$i]['nexthop_id'] . "\">" . $domain_properties[$i]['nexthop'] . "</a></td>";
