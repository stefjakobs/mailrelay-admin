...
if (authentication_has_role('global-admin')) {
   ...
   if ($CONF['relay'] == 'YES') {
      $submenu_domain .= _menulink("list-user-transport.php", $PALANG['pAdminMenu_list_user']);
      $submenu_domain .= _menulink("create-user-transport.php", $PALANG['pAdminMenu_create_user']);
      $submenu_transport = _menulink("create-transport.php", $PALANG['pAdminMenu_create_transport']);
   }
   if ($CONF['restriction'] == 'YES') {
      $submenu_restriction = _menulink("create-action.php", $PALANG['pAdminMenu_create_action']);
      $submenu_restriction .= _menulink("list-restriction.php", $PALANG['pAdminMenu_list_restriction']);
      $submenu_restriction .= _menulink("create-restriction.php", $PALANG['pAdminMenu_create_restriction']);
   }
} else {
    $submenu_domain = "";
   if($CONF['relay'] == 'YES') {
...
if ($CONF['relay'] == 'YES') {
   print _menulink("list-transport.php", $PALANG['pAdminMenu_list_transport'], $submenu_transport);
}
if ($CONF['restriction'] == 'YES') {
   print _menulink("list-action.php", $PALANG['pAdminMenu_list_action'], $submenu_restriction);
}
