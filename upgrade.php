...
function upgrade_738_mysql() {
    # add tables for the gateway management functionality (RELAY = YES)
    db_query_parsed("
        CREATE TABLE {IF_NOT_EXISTS} " . table_by_key('transport') . "(
            id SERIAL,
            nexthop VARCHAR(255) {LATIN1} NOT NULL,
            description VARCHAR(255) {LATIN1} NOT NULL,
            created datetime NOT NULL DEFAULT '0000-00-00 00:00:00',
            modified datetime NOT NULL DEFAULT '0000-00-00 00:00:00',
            PRIMARY KEY(id)
        ) {MYISAM} {LATIN1} COMMENT='Postfix Admin - Transport';
    ");
    db_query_parsed("
        CREATE TABLE {IF_NOT_EXISTS} " . table_by_key('transport_map') . "(
            id SERIAL,
            transport BIGINT NOT NULL,
            address VARCHAR(255) NOT NULL,
            PRIMARY KEY(address)
        ) {MYISAM} {LATIN1} COMMENT='Postfix Admin - Transport Map';
    ");
    db_query_parsed("
        CREATE TABLE {IF_NOT_EXISTS} " . table_by_key('user_transport') . "(
            id SERIAL,
            address VARCHAR(255) NOT NULL,
            ...
            active TINYINT(1) NOT NULL DEFAULT '1',
            PRIMARY KEY(address)
        ) {MYISAM} {LATIN1} COMMENT='Postfix Admin - User Transport';
    ");
    db_query_parsed("
        CREATE TABLE {IF_NOT_EXISTS} " . table_by_key('action') . "(
            id SERIAL,
            action VARCHAR(255) NOT NULL DEFAULT 'DUNNO',
            description VARCHAR(255) NOT NULL,
            created DATETIME NOT NULL DEFAULT '0000-00-00 00:00:00',
            modified DATETIME NOT NULL DEFAULT '0000-00-00 00:00:00',
            PRIMARY KEY(id)
    ...
    ");
    db_query_parsed("
        CREATE TABLE {IF_NOT_EXISTS} " . table_by_key('restriction') . "(
            id SERIAL,
...
function upgrade_738_pgsql() {
    # add tables for the gateway management functionality (RELAY = YES)
    if(!_pgsql_object_exists(table_by_key('transport'))) {
        db_query_parsed('
        CREATE TABLE ' . table_by_key('transport') . ' (
             id integer PRIMARY KEY NOT NULL,
             nexthop character varying(255) NOT NULL,
             description character varying(255) NOT NULL,
             created timestamp with time zone default now(),
             modified timestamp with time zone default now()
            );
        COMMENT ON TABLE ' . table_by_key('transport') . ' IS \'Postfix Admin - Transport\';
        ');
    }
    if(!_pgsql_object_exists(table_by_key('transport_map'))) {
        db_query_parsed('
        CREATE TABLE ' . table_by_key('transport_map') . ' (
             id integer NOT NULL default 0,
             transport BIGINT NOT NULL REFERENCES ' . table_by_key('transport') . '(id),
             address character varying(255) PRIMARY KEY NOT NULL
            );
        COMMENT ON TABLE ' . table_by_key('transport_map') . ' IS \'Postfix Admin - Transport Map\';
        ');
    }
    if(!_pgsql_object_exists(table_by_key('user_transport'))) {
        db_query_parsed('
        CREATE TABLE ' . table_by_key('user_transport') . ' (
             id integer NOT NULL default 0,
             address character varying(255) PRIMARY KEY NOT NULL,
             description character varying(255) NOT NULL,
             domain character varying(255) NOT NULL,
        ...
        COMMENT ON TABLE ' . table_by_key('user_transport') . ' IS \'Postfix Admin - User Transport\';
        ');
    }
    if(!_pgsql_object_exists(table_by_key('action'))) {
        db_query_parsed('
        CREATE TABLE ' . table_by_key('action') . ' (
             id integer PRIMARY KEY,
             action character varying(255) NOT NULL DEFAULT \'DUNNO\',
             description character varying(255) NOT NULL,
             created timestamp with time zone default now(),
             modified timestamp with time zone default now()
            );
        COMMENT ON TABLE ' . table_by_key('action') . ' IS \'Postfix Admin - Action\';
        ');
    }
    if(!_pgsql_object_exists(table_by_key('restriction'))) {
        db_query_parsed('
        CREATE TABLE ' . table_by_key('restriction') . ' (
             id integer PRIMARY KEY,
             address character varying(255) NOT NULL DEFAULT \'\',
             action integer NOT NULL REFERENCES ' .table_by_key('action') .'(id), 
             description character varying(255) NOT NULL,
             policy character varying(255) NOT NULL DEFAULT \'\',
             domain character varying(255) NOT NULL DEFAULT \'ALL\' REFERENCES '
                 . table_by_key('domain') .',
             created timestamp with time zone default now(),
             modified timestamp with time zone default now(),
             active boolean NOT NULL default true
            );
        COMMENT ON TABLE ' . table_by_key('restriction') . ' IS \'Postfix Admin - Restrtiction\';
        ');
    }
