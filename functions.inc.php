...
function check_domain ($domain)
{
    ...
    if (!preg_match ('/^(([-0-9A-Z]+\.)+' . '([0-9A-Z]){2,6}$)|(^localhost$)/i', ($domain)))
    {
...
//
// check_subdomain
// Action: Checks if a string ends with a dot
// Call: check_subdomain (string domain)
function check_subdomain ($subdomain)
{
    global $CONF;
    global $PALANG;

    $result = preg_match("/\.$/", trim($subdomain)); 
    if ($result) 
    {
        flash_error(sprintf($PALANG['pInvalidSubdomain'], htmlentities($subdomain)));
        return false;
    } else 
    {
        return true;
...
//
// check_transport
// Action: Checks if a transport is valid and returns TRUE if this is the case.
// Call: check_transport (string transport)
function check_transport ($transport)
{
    global $CONF;
    global $PALANG;

    # check the proper format of the transport
    if (!preg_match ('/^[\w\d_-]*:'			# protocol: 
                   . '(\[([-\w\d]+\.)+([\w\d]){2,6}\]|'	# [domain]
                   . '([-\w\d]+\.)+([\w\d]){2,6}|'	# domain
                   . '(\d{1,3}\.){3}\d{1,3}|'		# IP
                   . '\d\.\d\.\d.*|'			# X.Y.Z something
                   . 'localhost)?$/i', trim($transport)))
    {
        flash_error(sprintf($PALANG['pInvalidTransportRegex'], htmlentities($transport)));
        return false;
    }
    # remove the transport: part from $transport
    $transport = preg_replace('/^[\w\d_-]*:/', '', trim($transport));
    # remove possible square brackets 
    $transport = preg_replace('/[\[\]]/', '', trim($transport));
 
    if ($transport != NULL && !preg_match ('/^(\d{1,3}\.){3}\d{1,3}$|^\d\.\d\.\d/', trim($transport)))
    {
        if ( check_domain($transport) ) {
            return true;
        }
        else {
            return false;
...
function get_domain_properties ($domain)
{
    ...
    global $table_transport, $table_transport_map;
    $list = array ();
    ...
    $query="SELECT $table_domain.*, $table_transport.id, $table_transport.nexthop
            FROM $table_domain 
            LEFT JOIN $table_transport_map 
            ON $table_transport_map.address = $table_domain.domain
            LEFT JOIN $table_transport 
            ON $table_transport.id = $table_transport_map.transport
            WHERE $table_domain.domain='$domain'";
    if ('pgsql'==$CONF['database_type'])
    {
        $query="SELECT $table_domain.*, $table_transport.id, $table_transport.nexthop,
                EXTRACT(epoch FROM created) AS uts_created, EXTRACT(epoch FROM modified) AS uts_modified 
                FROM $table_domain 
                LEFT JOIN $table_transport_map
                ON $table_transport_map.address = $table_domain.domain
                LEFT JOIN $table_transport
    ...
    $list['nexthop'] = $row['nexthop'];
    $list['nexthop_id'] = $row['id'];
...
// Action: Get all the properties of a transport.
// Call: get_transport_properties (int id)
//
function get_transport_properties ($id)
{
    global $CONF;
    global $table_transport, $table_transport_map, $table_domain;
    $list = array ();

    $query="SELECT * FROM $table_transport WHERE id='$id'";
    if ($CONF['database_type'] == "pgsql")
    {
        $query=" SELECT *, EXTRACT(epoch FROM created) AS uts_created, EXTRACT(epoch FROM modified) AS uts_modified FROM $table_transport WHERE id='$id' ";
    }
    $result = db_query ($query);
    $row = db_array ($result['result']);
    $list['description'] = $row['description'];
    ...
    # fetch a list of domains which use this transport
    $query="SELECT address FROM $table_transport_map
            WHERE transport='$id'
            ORDER BY address" ;
    $result = db_query ($query);
    while ($row = db_array ($result['result'])) {
        if (isset($first)) {
            $list['domains'] = $list['domains'] . "<br>" . $row['address'];
        } else {
            $list['domains'] = $row['address'];
            $first = 1;
        }
    }
...
//
// get_user_properties
// Action: Get all the properties of a user transport.
// Call: get_user_properties (str address)
//
function get_user_properties ($address)
{
    global $CONF;
    global $table_transport_map, $table_user_transport;
    $list = array ();

    $query="SELECT $table_user_transport.*, $table_transport_map.transport FROM $table_user_transport
            LEFT JOIN $table_transport_map ON $table_transport_map.address = $table_user_transport.address
            WHERE $table_user_transport.address='$address'";
    if ($CONF['database_type'] == "pgsql")
    {
        $query="SELECT $table_user_transport.*, $table_transport_map.transport,
                EXTRACT(epoch FROM created) AS uts_created, EXTRACT(epoch FROM modified) AS uts_modified
...
//
// get_restriction_properties
// Action: Get all the properties of a restriction.
// Call: get_restriction_properties (int id)
//
function get_restriction_properties ($id)
{
    global $CONF;
    global $table_restriction, $table_action;
    $list = array ();

    $query="SELECT $table_restriction.*, $table_action.action AS action_name
            FROM $table_restriction
            LEFT JOIN $table_action ON $table_action.id = $table_restriction.action
            WHERE $table_restriction.id='$id'
           ";
    if ($CONF['database_type'] == "pgsql")
    {
        $query="SELECT $table_restriction*, $table_action.action AS action_name
                EXTRACT(epoch FROM created) AS uts_created, 
                EXTRACT(epoch FROM modified) AS uts_modified FROM $table_restriction
                LEFT JOIN $table_action ON $table_action.id = $table_restriction.action
                WHERE id='$id'
               ";
    }
    ...
    $list['action_name'] = $row['action_name'];
    $list['description'] = $row['description'];
    $list['policy'] = $row['policy'];
    $list['domain'] = $row['domain'];
    $list['created'] = $row['created'];
    $list['modified'] = $row['modified'];
    $list['active'] = $row['active'];
...
// Action: Get all the properties of a action.
// Call: get_action_properties (int id)
//
function get_action_properties ($id)
{
    global $CONF;
    global $table_action, $table_restriction;
    $list = array ();

    $query="SELECT * FROM $table_action WHERE id='$id'";
    if ($CONF['database_type'] == "pgsql")
    {
        $query=" SELECT *, EXTRACT(epoch FROM created) AS uts_created, EXTRACT(epoch FROM modified) AS uts_modified FROM $table_action WHERE id='$id' ";
    }
    $result = db_query ($query);
    $row = db_array ($result['result']);
    $list['description'] = $row['description'];
    ...
    # fetch a list of restrictions which use this action
    $query="SELECT address FROM $table_restriction
            WHERE action='$id'
            ORDER BY address" ;
    $result = db_query ($query);
    while ($row = db_array ($result['result'])) {
        if (isset($first)) {
            $list['restrictions'] = $list['restrictions'] . "<br>" . $row['address'];
        } else {
            $list['restrictions'] = $row['address'];
            $first = 1;
        }
    }
...
function domain_exist ($domain)
{
    ...
    {
        flash_error(sprintf($PALANG['pInvalidDomainExist'], htmlentities($domain)));
        return true;
    }
}
...
//
// transport_exist
// Action: Checks if the transport already exists.
// Call: transport_exist (string domain, $id)
//
function transport_exist ($transport, $id)
{
    global $table_transport;
    global $PALANG;

    $result = db_query("SELECT 1 FROM $table_transport WHERE nexthop='$transport' AND NOT id='$id'");
    if ($result['rows'] != 1)
    {
        return false;
    }
    ...
    {
        flash_error(sprintf($PALANG['pInvalidTransportExist'], htmlentities($transport)));
        return true;
    }
}
...
//
// user_exist
// Action: Checks if the user transport already exists.
// Call: user_exist (string address)
//
function user_exist ($address)
{
    global $table_user_transport;
    global $PALANG;

    $result = db_query("SELECT 1 FROM $table_user_transport WHERE address='$address'");
    if ($result['rows'] != 1)
    {
        return false;
    }
    ...
    {
        flash_error(sprintf($PALANG['pInvalidUserExist'], htmlentities($address)));
        return true;
    }
}
...
//
// action_exist
// Action: Checks if the action already exists.
// Call: action_exist (string action, int ID)
//
function action_exist ($action, $id)
{
    global $table_action;
    global $PALANG;

    $result = db_query("SELECT 1 FROM $table_action WHERE action='$action' AND NOT id='$id'");
    if ($result['rows'] != 1)
    {
        return false;
    }
    ...
    {
        flash_error(sprintf($PALANG['pInvalidActionExist'], htmlentities($action)));
        return true;
    }
}
...
//
// restriction_exist
// Action: Checks if the restriction already exists.
// Call: restriction_exist (string address, string policy, int id)
//
function restriction_exist ($address, $policy, $id)
{
    global $table_restriction;
    global $PALANG;

    $result = db_query("SELECT 1 FROM $table_restriction "
                      ."WHERE address='$address' AND policy='$policy' AND NOT id='$id'");
    if ($result['rows'] != 1)
    {
        return false;
    }
    ...
    {
        flash_error(sprintf($PALANG['pInvalidRestrictionExist'], htmlentities($address), htmlentities($policy)));
        return true;
    }
}
...
function db_log ($username,$domain,$action,$data)
{
    global $CONF;
    global $table_log;
    $REMOTE_ADDR = preg_replace('/::ffff:/', '', $_SERVER['REMOTE_ADDR']);

    $action_list = array('create_alias', 'create_alias_domain', 'create_transport', 'create_user_transport', 'create_action', 'create_restriction', 'create_domain', 'delete_alias', 'delete_alias_domain', 'delete_transport', 'delete_user_transport', 'delete_action', 'delete_restriction', 'delete_domain', 'edit_alias', 'create_mailbox', 'delete_mailbox', 'edit_mailbox', 'edit_alias_state', 'edit_alias_domain_state', 'edit_mailbox_state', 'edit_password', 'edit_transport', 'edit_user_transport', 'edit_action', 'edit_restriction', 'edit_domain');
    ...
    {
        $logdata = array(
            'username'  => "$username<br>($REMOTE_ADDR)",
            'domain'    => $domain,
            'action'    => $action,
            'data'      => $data,
        );
...
$table_transport = table_by_key ('transport');
$table_transport_map = table_by_key ('transport_map');
$table_user_transport = table_by_key ('user_transport');
$table_restriction = table_by_key ('restriction');
$table_action = table_by_key ('action');
