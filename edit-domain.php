...
if ($_SERVER['REQUEST_METHOD'] == "GET")
{
    if (isset ($_GET['domain']))
    {
        ...
        $tNexthop_id = $domain_properties['nexthop_id'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if ($result['rows'] == 1)
    {
        $log = "$sqlBackupmx : $sqlActive";
        if ($CONF['relay'] == 'YES' )
        {
            $query = "UPDATE $table_transport_map 
                      SET transport=$fNexthop_id
            ...
            {
                $tMessage = $PALANG['pAdminEdit_transport_result_error'] . "<br />($fNexthop_id)<br />";
                $error = 1;
            }
        }
        if (!defined($error)) {
            db_log ($SESSID_USERNAME, $domain, 'edit_domain', $log );
